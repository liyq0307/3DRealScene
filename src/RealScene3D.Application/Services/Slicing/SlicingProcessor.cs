...
public class SlicingProcessor : ISlicingProcessor
{
    ...
    private SimpleMesh MeshTToSimpleMesh(MeshT meshT)
    {
    ...
    private MeshT SimpleMeshToMeshT(SimpleMesh simpleMesh, MeshT? originalMesh = null)
    {
        // 转换顶点
    ...
    private List<MeshT> GenerateLODMeshes(MeshT originalMesh, SlicingConfig config)
    {
        ...
        // MeshT转换为SimpleMesh便于简化
        SimpleMesh mesh = MeshTToSimpleMesh(originalMesh);
        ...
        for (int index = 0; index < qualities.Length; index++)
        {
            try
            {
                ...
                var simplifiedMesh = MeshDecimation.DecimateMesh(algorithm, mesh, targetTriangleCount);

                // 将简化后的 SimpleMesh 转换回 MeshT
                var lodMesh = SimpleMeshToMeshT(simplifiedMesh, originalMesh);
