...
public static class MeshDecimation
{
    ...
    /// <param name="targetTriangleCount">目标三角形数量。</param>
    /// <returns>简化的网格。</returns>
    public static IMesh DecimateMesh(IMesh mesh, int targetTriangleCount)
    {
        return DecimateMesh(Algorithm.Default, mesh, targetTriangleCount);
    }
    ...
    /// <param name="targetTriangleCount">目标三角形数量。</param>
    /// <returns>简化的网格。</returns>
    public static IMesh DecimateMesh(Algorithm algorithm, IMesh mesh, int targetTriangleCount)
    {
        if (mesh == null)
    ...
    /// <param name="targetTriangleCount">目标三角形数量。</param>
    /// <returns>简化的网格。</returns>
    public static IMesh DecimateMesh(DecimationAlgorithm algorithm, IMesh mesh, int targetTriangleCount)
    {
        if (algorithm == null)
    ...
    /// <param name="mesh">要简化的网格。</param>
    /// <returns>简化的网格。</returns>
    public static IMesh DecimateMeshLossless(IMesh mesh)
    {
        return DecimateMeshLossless(Algorithm.Default, mesh);
    }
    ...
    /// <param name="mesh">要简化的网格。</param>
    /// <returns>简化的网格。</returns>
    public static IMesh DecimateMeshLossless(Algorithm algorithm, IMesh mesh)
    {
        if (mesh == null)
            throw new ArgumentNullException(nameof(mesh));
    ...
    /// <param name="mesh">要简化的网格。</param>
    /// <returns>简化的网格。</returns>
    public static IMesh DecimateMeshLossless(DecimationAlgorithm algorithm, IMesh mesh)
    {
        if (algorithm == null)
            throw new ArgumentNullException(nameof(algorithm));
        else if (mesh == null)
    ...
    /// <summary>
    /// 将 IMesh 转换为 SimpleMesh
    /// 完整转换包括顶点、UV坐标和法线
    /// 支持有纹理和无纹理的网格
    /// </summary>
    /// <param name="mesh">IMesh 对象</param>
    /// <returns>SimpleMesh 对象</returns>
    public static SimpleMesh ToSimpleMesh(IMesh mesh)
    {
        bool hasTexture = mesh.HasTexture;

        if (hasTexture)
        {
            // 有纹理：由于顶点索引和纹理索引分离，需要展开为统一的顶点数组
            ...
            _logger.LogDebug("带纹理网格 → SimpleMesh 转换完成：原始顶点={OrigVerts}/{OrigUVs}, 展开后顶点={ExpandedVerts}, 子网格数={SubMeshCount}",
                mesh.Vertices.Count, mesh.TextureVertices?.Count, vertices.Length, simpleMesh.SubMeshCount);
        ...
        else
        {
            ...
            {
                var face = mesh.Faces[faceIndex];
                indices.Add(face.IndexA);
                indices.Add(face.IndexB);
                indices.Add(face.IndexC);
            }

            // 创建 SimpleMesh
            var simpleMesh = new SimpleMesh(vertices, indices.ToArray());
            ...
            _logger.LogDebug("无纹理网格 → SimpleMesh 转换完成：顶点={VertexCount}, 面={FaceCount}",
                vertices.Length, mesh.Faces.Count);
    ...
    /// <summary>
    /// 将 SimpleMesh 转换为 IMesh
    /// 用于简化流程中需要将简化后的网格转换回 IMesh 格式
    /// 根据原始网格类型返回 Mesh 或 MeshT
    /// </summary>
    /// <param name="simpleMesh">SimpleMesh 对象</param>
    /// <param name="originalMesh">原始 IMesh 对象，用于判断网格类型和复制材质信息（可选）</param>
    /// <returns>Mesh 或 MeshT 对象</returns>
    public static IMesh ToMesh(SimpleMesh simpleMesh, IMesh? originalMesh = null)
    {
        // 判断是否需要纹理支持
        bool hasTexture = originalMesh?.HasTexture ?? false;
        ...
        if (hasTexture)
        {
            ...
            for (int subMeshIndex = 0; subMeshIndex < subMeshCount; subMeshIndex++)
            {
                ...
                for (int i = 0; i < indices.Length; i += 3)
                {
                    ...
                    faces.Add(new Face(indexA, indexB, indexC,
        ...
        else
        {
            ...
            _logger.LogDebug("SimpleMesh → Mesh 转换完成：顶点={VertexCount}, 面={FaceCount}",
