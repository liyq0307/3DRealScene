      ...
      <div class="form-container">
        ...
        <section class="form-section">
          <h2 class="section-title">
            ...
            场景对象
            <span v-if="pendingObjects.length > 0" class="object-count">({{ pendingObjects.length }})</span>
          </h2>

          <!-- 对象列表 -->
          <div v-if="pendingObjects.length > 0" class="objects-list">
            <div v-for="(obj, index) in pendingObjects" :key="index" class="object-item">
              <div class="object-info">
                <span class="object-icon">{{ getObjectIcon(obj.objectType) }}</span>
                <div class="object-details">
                  <h4 class="object-name">{{ obj.name }}</h4>
                  <p class="object-type">{{ obj.objectType }}</p>
                </div>
                <button @click="removeObject(index)" class="btn-remove-object" type="button" title="删除">
                  ✕
                </button>
              </div>
    ...
    <Modal v-model="showObjectDialog" title="添加场景对象" size="lg">
      <div class="object-dialog">
        ...
        <div class="transform-section">
          <h3 class="transform-title">位置设置</h3>
          <div class="form-group">
            ...
            <p class="form-hint">
              💡 提示：位置使用 WGS84 坐标系。对于 3D Tiles 模型，通常使用默认位置 (0, 0, 0) 即可。
            </p>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        <button @click="showObjectDialog = false" class="btn btn-secondary">取消</button>
        <button @click="addObject" class="btn btn-primary" :disabled="!isObjectFormValid">添加</button>
      </template>
    </Modal>
  </div>
</template>
...
const addObject = () => {
  ...
  const newObject = {
    ...objectForm.value,
    file: selectedObjectFile.value,
    files: selectedObjectFiles.value
  }

  pendingObjects.value.push(newObject)
  showSuccess(`场景对象 "${objectForm.value.name}" 已添加到列表`)
...
const removeObject = (index: number) => {
  const obj = pendingObjects.value[index]
  pendingObjects.value.splice(index, 1)
  showSuccess(`场景对象 "${obj.name}" 已从列表中移除`)
}
...
const saveScene = async () => {
  ...
  try {
    ...
    const createdScene = await sceneService.createScene(sceneData, userId)
    const sceneId = createdScene.id || createdScene.sceneId
    
    showSuccess('场景创建成功！')

    // 如果有待保存的场景对象，逐个创建
    if (pendingObjects.value.length > 0) {
      showSuccess(`正在创建 ${pendingObjects.value.length} 个场景对象...`)

      for (const obj of pendingObjects.value) {
        try {
          let finalModelPath = obj.modelPath

          // 处理批量文件上传
          if (obj.files && obj.files.length > 0) {
            try {
              showSuccess(`正在上传 ${obj.files.length} 个文件...`)
              const uploadResult = await fileService.uploadFilesBatch(obj.files, 'models-3d', obj.name)
              ...
              // 查找主模型文件的上传结果
              const objFile = obj.files.find((f: File) => f.name.toLowerCase().endsWith('.obj'))
              const glbFile = obj.files.find((f: File) => f.name.toLowerCase().endsWith('.glb'))
              const gltfFile = obj.files.find((f: File) => f.name.toLowerCase().endsWith('.gltf'))
              ...
              showError(`对象 "${obj.name}" 的文件上传失败，使用原始路径`)
            }
          }
          // 处理单个文件上传
          else if (obj.file) {
            try {
              const uploadResult = await fileService.uploadFile(obj.file, 'models-3d')
          ...
          console.log(`场景对象 "${obj.name}" 创建成功`)
        } catch (error) {
          console.error(`创建场景对象 "${obj.name}" 失败:`, error)
          showError(`场景对象 "${obj.name}" 创建失败`)
        }
      }

      showSuccess(`场景和 ${pendingObjects.value.length} 个对象创建成功！`)
    }
