<template>
  <div class="scene-objects-add-page">
    <!-- 页面头部 -->
      ...
      <div class="form-container">
        ...
        <section class="form-section">
          ...
          <div class="form-group">
            <label>目标场景 <span class="required">*</span></label>
            <select v-model="selectedSceneId" class="form-input" @change="handleSceneChange">
            ...
            <div class="form-group">
              ...
              <div class="model-path-selector">
                <input
                  ...
                  type="text"
                  class="form-input"
                  placeholder="输入本地文件路径（如：C:\models\model.glb）或URL"
                />
...
const selectLocalFile = async () => {
  ...
  if ('showOpenFilePicker' in window && window.showOpenFilePicker) {
    try {
      ...
      // 尝试获取文件路径
      // 注意：浏览器出于安全考虑，无法直接获取完整路径
      // 但我们可以保存文件名，让用户手动补充路径
      const fileName = file.name
      objectForm.value.modelPath = fileName
      showSuccess(`已选择文件: ${fileName}。如需完整路径，请手动编辑路径字段。`)
...
const handleMultipleFilesSelect = (event: Event) => {
  ...
  if (objFile) {
    objectForm.value.modelPath = `批量上传: ${objFile.name} + ${finalFiles.length - 1} 个相关文件`
    showSuccess(`已选择 ${finalFiles.length} 个文件，主模型: ${objFile.name}`)
  } else {
    objectForm.value.modelPath = `批量上传: ${finalFiles.length} 个文件`
...
const saveObject = async () => {
  ...
  try {
    ...
    if (selectedFiles.value.length > 0) {
      ...
      console.log('[SceneObjectsAddPage] 准备批量上传文件:', selectedFiles.value.map(f => ({
        name: f.name,
        size: f.size,
        type: f.type
      })))
      ...
      try {
        ...
        console.log('[SceneObjectsAddPage] 批量上传API响应:', uploadResult)
        
        if (!uploadResult.success) {
          throw new Error(`批量上传失败: ${uploadResult.failedCount} 个文件上传失败`)
        }
        
        console.log(`[SceneObjectsAddPage] 批量上传成功: ${uploadResult.successCount}/${uploadResult.totalFiles}`)
        ...
        if (objFile && uploadResult.results) {
          ...
          if (objResult) {
            ...
            console.log('[SceneObjectsAddPage] OBJ主模型路径:', finalModelPath)
          }
          ...
          if (glbResult) {
            ...
            console.log('[SceneObjectsAddPage] GLB主模型路径:', finalModelPath)
          }
          ...
          if (gltfResult) {
            ...
            console.log('[SceneObjectsAddPage] GLTF主模型路径:', finalModelPath)
          }
          ...
          console.log('[SceneObjectsAddPage] 使用第一个文件作为主模型:', finalModelPath)
        }
        
        showSuccess(`批量上传成功！共 ${uploadResult.successCount} 个文件`)
      } catch (uploadError) {
        console.error('[SceneObjectsAddPage] MinIO上传失败，尝试使用文件句柄:', uploadError)
        ...
        if (selectedFileHandle.value) {
          try {
            ...
            console.log('[SceneObjectsAddPage] 使用文件句柄路径:', finalModelPath)
            showSuccess('MinIO上传失败，已保存文件句柄作为备选方案')
          } catch (handleError) {
            console.error('[SceneObjectsAddPage] 保存文件句柄也失败:', handleError)
    ...
    console.log('创建场景对象数据:', objectData)
    ...
    console.error('添加场景对象失败:', error)
    const errorMessage = error.response?.data?.message || error.message || '添加场景对象失败'
    showError(`添加场景对象失败: ${errorMessage}`)
  } finally {
...
onMounted(async () => {
  ...
  // 从query参数获取场景ID
  const sceneIdFromQuery = route.query.sceneId as string
  if (sceneIdFromQuery) {
