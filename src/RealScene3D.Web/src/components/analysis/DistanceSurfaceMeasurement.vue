<template>
  <div class="distance-surface-measurement">
    <div class="section-title">贴地距离测量</div>
    <div class="btn-group">
    ...
    <div v-if="result" class="result-panel">
      <div class="result-item"><span class="label">贴地距离:</span><span class="value highlight">{{ formatDistance(result.distance) }}</span></div>
    </div>
  </div>
</template>
...
const result = ref<{ distance: number } | null>(null)

onMounted(() => { if (!isReady.value) init() })

async function handleMeasure() {
  isMeasuring.value = true
  result.value = await measureDistanceSurface()
...
<style scoped>
.distance-surface-measurement { padding: 12px; }
