<template>
  <div class="building-spacing-analysis">
    <div class="section-title">多建筑间距分析</div>
    ...
    <div class="form-group">
      <label>最小间距(m)</label>
      <input v-model.number="minDistance" type="number" step="1" />
    </div>
    <div class="btn-group">
...
async function handleDraw() {
  ...
  if (result) {
    store.addResult({ type: 'building-spacing', name: '建筑间距分析', data: { areaType: areaType.value, minDistance: minDistance.value, regionPositions: result.positionShow || result.positions }, visible: true })
  }
...
<style scoped>
.building-spacing-analysis { padding: 12px; }
...
.form-group label { width: 80px; font-size: 12px; color: #aaa; }
.form-group select, .form-group input { flex: 1; padding: 4px 8px; background: #2a2a2a; border: 1px solid #444; color: #e0e0e0; border-radius: 4px; font-size: 12px; }
