...
public:
	...
	/**
	 * @brief 简化网格几何体
	 * @param pGeometry 输入/输出的网格几何体
	 * @param params 网格简化参数
	 * @return true=成功, false=失败
	 * @note 该函数会修改输入的 osg::Geometry 对象以应用简化。
	 * 		 调用者负责确保输入几何体具有有效的顶点和索引数据。
	 *    	 如果几何体缺少位置属性，简化将失败。
	 *   	 该函数假设输入几何体的索引是三角形列表格式，目前仅支持三角形列表的简化。
	 *  	 调用者负责在调用前后管理几何体的内存。
	 */
	static bool SimplifyMeshGeometry(osg::Geometry* pGeometry, const SimplificationParams& params);

	/**
	 * @brief 使用 Draco 压缩网格几何体
	 * @param pGeometry 输入的网格几何体
	 * @param params Draco 压缩参数
	 * @param compressedData 输出的压缩数据
	 ...
	 * @param pTexture 输入的 OSG 纹理对象
	 * @param imageData 输出的图像数据
	 * @param strMimeType 输出的 MIME 类型字符串
	 * @param bEnableTextureCompress 是否启用纹理压缩
	 * @return true=成功, false=失败
	 * @note 如果启用纹理压缩，图像数据将以 KTX2 格式输出，MIME 类型为 "image/ktx2"。
	 *       如果未启用纹理压缩，图像数据将以原始格式输出，MIME 类型将根据原始图像格式设置（如 "image/png" 或 "image/jpeg"）。
	 * 	 	 调用者负责管理输出图像数据的内存。
	 */
